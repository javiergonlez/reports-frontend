...
import type { LoginCredentials, ValidationError, ValidationResult } from '../types';
import './auth.css';
...
const LoginPage = (): React.JSX.Element => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    const validation: ValidationResult = validateLoginForm(credentials.email, credentials.password);
    ...
    try {
      ...
      const timeoutPromise = new Promise<never>((_, reject) => {
        setTimeout(() => reject(new Error('Tiempo de espera agotado. Verifica tu conexión a internet.')), 30000);
      });
